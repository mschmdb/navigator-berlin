...
const DraftSchema = v.object({
	...
	tags: v.pipe(
		v.array(v.pipe(v.string(), v.regex(/^[a-z0-9-]+$/))),
		v.maxLength(8)
	),
...
export function parseDraftResult(raw: unknown): { ok: true; value: DraftResult } | { ok: false; error: string } {
	const r = v.safeParse(DraftResultSchema, raw);
	if (r.success) return { ok: true, value: r.output };
	return { ok: false, error: r.issues.map((i) => `${i.path?.map((p) => p.key).join('.')}: ${i.message}`).join('; ') };
}
