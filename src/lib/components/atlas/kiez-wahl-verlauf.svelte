	...
	>
		<header class="space-y-1">
			...
			<p class="font-serif text-base text-ink-muted">
				Im Kiez {kiezName} verteilten sich die Stimmen bei den letzten Wahlen jeweils so,
				dass folgende Parteien pro Jahr den höchsten Anteil erreichten.
			</p>
		</header>
				...
				<li class="space-y-2" data-testid={`${testid}-row-${row.key}`}>
					...
					>
						{#each row.jahre as cell (cell.jahr)}
							<li
								class="flex flex-col gap-1.5 border border-rule rounded p-2.5 bg-bg"
								data-testid={`${testid}-${row.key}-${cell.jahr}`}
							>
								<span
									class="font-mono text-[10px] uppercase tracking-wide text-ink-muted tabular-nums"
								>
									{cell.jahr}
								</span>
								<span class="flex items-center gap-2">
									<span
										class="inline-block h-3 w-3 rounded-sm border border-ink/15 flex-shrink-0"
										style="background-color:{parteiColor(cell.parteiKurzname)};"
										aria-hidden="true"
									></span>
									<span class="font-sans text-sm font-medium text-ink truncate">
										{cell.parteiKurzname}
									</span>
								</span>
							</li>
