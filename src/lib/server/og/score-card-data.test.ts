...
describe('buildScoreCardData', () => {
	it('returns empty composite + 5 null dims when score is null', () => {
	...
	it('returns composite + 5 ADR-015 dim values (Ruhe / Grün & Hitze / Mobilität / Versorgung / Wohnschutz)', () => {
		const score = {
			composite: 43,
			ruheLuft: 27,
			gruenHitze: 31,
		...
		expect(result.dims.map((d) => d.label)).toEqual([
			'Ruhe',
			'Grün & Hitze',
			'Mobilität',
			'Versorgung',
		...
		expect(result.dims.map((d) => d.value)).toEqual([27, 31, 35, 62, 100]);
		// Sozialstruktur ist kein Score-Wert mehr (ADR-015)
	...
	it('handles partial null dims', () => {
		...
		expect(result.dims.map((d) => d.value)).toEqual([null, 40, null, 70, null]);
	});
});
