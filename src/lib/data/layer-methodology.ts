...
const LAYER_METHODOLOGY_SPECS: Record<string, LayerMethodologySpec> = {
	...
	'kiez-score-versorgung': {
		calculation:
			'Kita doppelt gemessen: Distanz zur nächsten Kita (Gewicht 0.15, Threshold 500 m) plus Plätze pro Kind 0-6 im Planungsraum (0.15). Der Pro-Kopf-Term summiert die gemeldeten Kita-Plätze (e_platz) im LOR und teilt durch die Kinder 0-6 aus dem Einwohner-Datensatz: ab 0.35 Plätzen pro Kind volle Punktzahl, linear darunter. Dazu Distanz zu Schule nach Schulart getrennt: Grundschule (0.15, 600 m) und weiterführende Schule (0.15, 1.200 m). Plus Spielplatz (0.15, 400 m). Plan-Krankenhaus (0.25, 2.000 m) zusätzlich nach Bettenkapazität gewichtet: ein großes Versorgungs-Klinikum zählt mehr als eine kleine Fachklinik. 0 m → 100, Threshold → 0, linear. Spielplätze (Polygone) nutzen den Geometrie-Mittelpunkt als POI-Punkt. Grünanlagen zählen seit der Score-Neuordnung unter Grün & Hitze.',
		aggregationLevel: 'lor-planungsraum',
